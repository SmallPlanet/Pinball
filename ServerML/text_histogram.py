...
def histogram(stream, weights, minimum=None, maximum=None, buckets=None, custbuckets=None, calc_msvd=True):
    ...
    if custbuckets:
        ...
        bucket_weights = [0 for x in range(len(boundaries))]
        ...
        bucket_weights = [0 for x in range(buckets)]
    ...
    for i in range(0,len(data)):
        ...
        for bucket_postion, boundary in enumerate(boundaries):
            if value <= boundary:
                ...
                bucket_weights[bucket_postion] += weight
                break
    
    for i in range(0,len(bucket_counts)):
        if bucket_counts[i] > 0:
            bucket_weights[i] /= bucket_counts[i]
